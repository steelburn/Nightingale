...
# Install Go
RUN set -eux; \
    wget -q "https://go.dev/dl/go${GO_VERSION}.linux-arm64.tar.gz" -O go.tar.gz; \
...
RUN update-alternatives --install /usr/bin/java java /usr/lib/jvm/java-21-openjdk-arm64/bin/java 1 && \
    update-alternatives --install /usr/bin/javac javac /usr/lib/jvm/java-21-openjdk-arm64/bin/javac 1
